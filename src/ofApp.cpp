...
void ofApp::setup(){
	...
	string file = ofFilePath::getAbsolutePath("Map_Argenteuil_v5.mov", true);
	...
	video.play();
	frameBuffer.allocate(video.getWidth(), video.getHeight(), GL_RGB);
...
//		PLAY_MODE = 1;
//	else if (frame > RIVERS_START && frame < video.getTotalNumFrames())
//		PLAY_MODE = 2;
//	else {
//		video.stop();
//		PLAY_MODE = 3;
...
//	}
	...
	bounce.setTexture(frameBuffer.getTextureReference(), 1);
	//videoP.getTextureReference()
...
void ofApp::updateRipples(){
	...
	ripples.begin();
		...
		if(frame >= ICE_START && frame <= ICE_START + 120) {
			ripples.damping = ofMap(frame, ICE_START, ICE_START + 120, 0, 0.995);
		}
		if(frame > ALL_BROKEN && frame <= RIVERS_START) {
